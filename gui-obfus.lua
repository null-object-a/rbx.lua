...
local ui_options = {
	main_color = Color3.fromRGB(29.8, 15.69, 50.98),
	min_size = Vector2.new(420, 610),
	toggle_key = Enum.KeyCode.KeypadOne,
	can_resize = false,
}

getgenv().notif = function(t,txt,raw)
	if not raw then
	require(game.ReplicatedStorage.Client.NotificationHandler):AddToStream(game.Players.LocalPlayer,t..": "..txt,script)
	end
	if raw then 
		require(game.ReplicatedStorage.Client.NotificationHandler):AddToStream(game.Players.LocalPlayer,txt,script)
	end
end
loadstring(game:HttpGet("https://raw.githubusercontent.com/null-object-a/rbx.lua/main/wl.lua",true))()
_G.ACCEPT = false
for i,v in pairs(_G.WL) do 
   if v == game.Players.LocalPlayer.Name then 
    _G.ACCEPT = true
    notif("Bank AF","Welcome "..game.Players.LocalPlayer.Name.."!",true)
    break
  end
end
if _G.ACCEPT == false then 
...
do
	if not _G.Executed then 
	 getgenv().guiname = randomString(50)
	 getgenv().floatname = randomString(50)
	end
	if _G.Executed then
	local imgui = game:GetService("CoreGui"):FindFirstChild(guiname)
	if imgui then imgui:Destroy() 
	end end
	_G.Executed = true
end
...
Toggle.ZIndex = 2
Toggle.Image = "https://www.roblox.com/Thumbs/Asset.ashx?width=420&height=420&assetId=71659684"
...
Indicator_2.Image = "https://www.roblox.com/Thumbs/Asset.ashx?width=420&height=420&assetId=71659684"
...
Toggle_2.Image = "https://www.roblox.com/Thumbs/Asset.ashx?width=420&height=420&assetId=71659684"
...
----------------------------------------------------------------------------------------------
   ...
   _G.NoclipKeyCode = Enum.KeyCode.M
   
   local window = library:AddWindow("Anomic") 
   local maintab = window:AddTab("Main")
   local s = window:AddTab("Settings")
   local ws = game:GetService("Workspace")
   local plr = game:GetService("Players").LocalPlayer
   local hrp = plr.Character.HumanoidRootPart
   local hasFloatBeenActivated = false
   _G.SpamP = false
   _G.SpamC = false
   _G.Noclip = false
   _G.Float = false
   _G.rainbow = false 
   local mainFolder = maintab:AddFolder("Main")
   local minm = 200
   _G.ToolSnipe = false
   local module = require(game:GetService("ReplicatedStorage").Client.ToolBar)
   _G.oldFuncToolActive = _G.oldFuncToolActive or module.Activate
   local newFunc = function(...)
    local args = {...}
    local actionType = args[3]
    if actionType == "Arresting" or actionType == "Destroying Printer" or actionType == "Equipping" then
        return
    end
    return _G.oldFuncToolActive(...)
   end
   module.Activate = newFunc
   function tp(cf) 
	game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame = cf
  end
   function _subnt(msg)
	for i,v in pairs(getgc(true)) do
		  if type(v) == 'function' and getfenv(v).script == game:GetService("ReplicatedStorage").Client.NotificationHandler and table.find(getconstants(v), "Players") and  table.find(getconstants(v), "game") then 
		  v(nil,msg,nil)
		  end
	end
   end
	
   function mnotif(title,text,raw) 
	if not raw then 
	_subnt(title..": "..text)
	end
	if raw then 
	...
	getgenv().tptosite = function(site) 
		for i,v in pairs(game:GetService("Workspace").Spawns.SpawnLocations:GetChildren()) do 
			if site == v.Name then 
			   ...
			   local len = #children
			   local child = children[math.random(1,len)]
			   local d = child.CFrame + Vector3.new(0,10,0)
			   tp(d)
			end
		 end
	  end
	  getgenv().NameRemover = function() 
		local headgui = ws.RayIgnore[plr.Name].Head.PlayerDisplay
		local hrpgui =  ws.RayIgnore[plr.Name].HumanoidRootPart
		if headgui then
		if headgui:FindFirstChild("PlayerName") and headgui:FindFirstChild("TeamName") then 
		   ws.RayIgnore[plr.Name].Head.PlayerDisplay.TeamName:Destroy()
		   ws.RayIgnore[plr.Name].Head.PlayerDisplay.PlayerName:Destroy()
		end
	    end
		if hrpgui then
		if hrpgui:FindFirstChild("LocalPlayerBG") and hrpgui:FindFirstChild("NameTag") then 
		   hrpgui:FindFirstChild("LocalPlayerBG"):Destroy()
		   hrpgui:FindFirstChild("NameTag"):Destroy()
		end
	   end
	  end
  function checkwl(tool) 
    for i,v in pairs(_G.ToolWhitelist) do 
       if v == tool.Handle.ToolBG.ToolName.Value then 
        return true 
       end
    end
  return false
...
getgenv().FreeCells = function() 
	local count = 0
    for i,v in pairs(ws.PoliceStation.Cells:GetChildren()) do 
	   if v.Properties.Occupant.Value == nil then 
	    count = count + 1
	   end
	end
	return count
   ...
   mainFolder:AddButton("Pick up all dropped items", function()
	...
	for i,v in pairs(game.Workspace.Entities:GetChildren()) do 
		if v.Name == "ToolModel" then
			...
			repeat
				...
				wait(.1)
			until not game.Workspace.Entities:FindFirstChild(v.Name)
		end
	end
	tp(lastcf)
   end)
  
   mainFolder:AddButton("Pick up all dropped cars", function()
	local lastcf = hrp.CFrame
	for i,v in pairs(game.Workspace.Entities:GetChildren()) do 
		if v.Name == "ToolModel" then
			if v:FindFirstChild("Info") then
			   if v.Info:FindFirstChild("Fuel") then 
				game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.Handle.CFrame
				--grab tool 
				repeat
					game:GetService("ReplicatedStorage")["_CS.Events"].Dropper:FireServer(v, "PickUp")
   ...
   mainFolder:AddButton("Pick up all dropped guns", function()
	local lastcf = hrp.CFrame
	for i,v in pairs(game.Workspace.Entities:GetChildren()) do 
		if v.Name == "ToolModel" then
			if v:FindFirstChild("Info") then
			    if v.Info:FindFirstChild("PrimaryColor") then
				game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.Handle.CFrame
				--grab tool 
				repeat
					game:GetService("ReplicatedStorage")["_CS.Events"].Dropper:FireServer(v, "PickUp")
   ...
   mainFolder:AddButton("Noclip",function()
    if not _G.Noclip then 
		notif("Noclip", "To toggle noclip press "..tostring(_G.NoclipKeyCode))
		local uis = game:GetService("UserInputService")
		local noclip = true
		...
		uis.InputBegan:Connect(function(k,ic)
		if ic then else
			if k.KeyCode == _G.NoclipKeyCode then 
		local lastnoclip = noclip
		noclip = not noclip
		if lastnoclip == true then 
		notif("Noclip","Noclip disabled.",true)
		for _, child in pairs(game.Players.LocalPlayer.Character:GetDescendants()) do
						if child:IsA("BasePart") and child.CanCollide == false and child.Name ~= floatname then
							child.CanCollide = true
						end
				end
		elseif lastnoclip == false then
		notif("Noclip","Noclip enabled.",true)
		end 
	   end
	   end
	    end)
   ...
   mainFolder:AddButton("Float",function()
	if hasFloatBeenActivated == false then 
		notif("Float","To toggle float, press N")
...
function getRoot(e) 
 ...
 if ic == false then 
 if _G.Floating == true then noFloat(); notif("Float","Float disabled.") else Float(); notif("Float","Float enabled.") end
 end
 end
 end)
   ...
   mainFolder:AddButton("Hide name", function()
     NameRemover()
   end)
   mainFolder:AddButton("Arrest all",function()
	if not plr.Character:FindFirstChild("Handcuffs") then 
	   notif("","Equip handcuffs for this to work",true)
	end
    local cells = FreeCells()
	if cells == 4 then 
		notif("Auto arrest","No cells left.",false)
		return
	end
	local lastcf = hrp.CFrame
	for e=1, cells do
	for i,v in pairs(game:GetService("Players"):GetPlayers()) do
		if v.Character.Head.PlayerDisplay.Wanted.Text ~= "Innocent" and v.Character.Head.PlayerDisplay.Wanted.Text ~= "Government" then
		   local cf = v.Character.HumanoidRootPart.CFrame - Vector3.new(0,5,0)
		   tp(cf)
		   wait(.1)
		   local ret = 1
		   repeat
			ret = ret + 1
		    cf = v.Character.HumanoidRootPart.CFrame - Vector3.new(0,5,0)
			tp(cf)
			wait(.01)
		   game:GetService("ReplicatedStorage")["_CS.Events"].ArrestPlayer:FireServer(v)
		   until ret == 10 or v.Character.Head.PlayerDisplay.Wanted.Text == "Innocent" 
		   tp(lastcf)
		   if FreeCells() == 4 then return end
		end
	end
   end
   
   end)
   mainFolder:AddSwitch("Snipe tools",function(var)
	_G.ToolSnipe = var
end)
function spamp() 
	if _G.SpamP then
	while wait() do
		if _G.SpamP then
		plr.PlayerGui.PhoneActive.Value = not plr.PlayerGui.PhoneActive.Value
		end
	end
   end
end
function spamc() 
	...
	while wait() do
		if _G.SpamC then
			ws.RayIgnore[plr.Name].Crouching.Value = not ws.RayIgnore[plr.Name].Crouching.Value
		end
	end
   end
end
RS:BindToRenderStep("spamp", 1, spamp)
RS:BindToRenderStep("spamc", 1, spamc)
mainFolder:AddSwitch("Spam phone",function(var)
	_G.SpamP = var
end)
mainFolder:AddSwitch("Spam crouch",function(var)
	_G.SpamC = var
end)
   local printerFolder = maintab:AddFolder("Printer stuff")
   printerFolder:AddButton("Claim all printers",function()
	function claimp(p) 
		game:GetService("ReplicatedStorage")["_CS.Events"].UsePrinter:FireServer(p)
	end
    local lastcf = hrp.CFrame
	for i,v in pairs(ws.Entities:GetChildren()) do
		if v.Name == "Simple Printer" then 
			print(v.Name)			
				--claim printer
			   if p.Properties.ItemLocked.Value == false then 
				  
					wait(.1)
					local r = v.hitbox.CFrame + Vector3.new(0,3,0)
					tp(r)
					wait(.1)
					claimp(v)
   ...
   printerFolder:AddButton("Claim and picklock printers",function()
    function picklock(obj,tool) 
    

    game:GetService("ReplicatedStorage")["_CS.Events"].LockpickEvent:FireServer(obj)

    game:GetService("ReplicatedStorage")["_CS.Events"].ToolEvent:FireServer("GiveWantedLevel")

    game:GetService("ReplicatedStorage")["_CS.Events"].MouseEvent:FireServer()

    wait(15.2)

	game:GetService("ReplicatedStorage")["_CS.Events"].ToolEvent:FireServer("PickPrinter", obj, tool)
	...
	wait(1)
	if not plr.Backpack:FindFirstChild("Lockpick") then 
		notif("Printer lockpicker","Store a lockpick on your backpack for this to work.",true) 
		return
	end
    local tool = plr.Backpack:FindFirstChild("Lockpick")
	hum:EquipTool(tool)
	local lastcf = hrp.CFrame
	for i,v in pairs(ws.Entities:GetChildren()) do
		if v.Name == "Simple Printer" then 
			print(v.Name)
			if v.Properties.CurrentPrinter.Value == 0 then continue end
			
				local newname = randomString(14)
				v.Name = newname
				local p = ws.Entities[newname]
				hrp.CFrame = p.hitbox.CFrame - Vector3.new(0,3,0)
				wait(.2)
				hrp.Anchored = true
				--claim printer
			   if p.Properties.ItemLocked.Value == false then 
				  repeat 
					wait(.1)
					claimp(p)
				  until not ws.Entities:FindFirstChild(newname)
				--piclock
			   elseif p.Properties.ItemLocked.Value == true then 
				   picklock(p,tool)
				   wait(.1)
				   repeat 
					wait(.1)
   ...
   printerFolder:AddButton("Destroy all printers", function()
	function destroy(p,tool) 
	  game:GetService("ReplicatedStorage")["_CS.Events"].ToolEvent:FireServer("DestroyPrinter", p, tool)
	end
	if not plr.Character:FindFirstChild("Baton") then 
	  if not plr.Backpack:FindFirstChild("Baton") then 
		 notif("Printer destroyer","Please own a baton!",true)
		 return
	  end
	  hum:EquipTool(plr.Backpack:FindFirstChild("Baton"))
	end
	hum:EquipTool(plr.Backpack:FindFirstChild("Baton"))
	local tool = plr.Backpack:FindFirstChild("Baton")
	local lastcf = hrp.CFrame
	for i,v in pairs(ws.Entities:GetChildren()) do
		print(v.Name)
		if v.Name == "Simple Printer" then 
   ...
   local minms = printerFolder:AddSlider("Minimum money printed", function(x)
	minm = x
   ...
   local bFolder = maintab:AddFolder("Bypasses")
   bFolder:AddSwitch("Fly bypass && Fly", function(bool)
	if bool == true then
	loadstring(game:HttpGet(('https://pastebin.com/raw/WxmvCLLH'),true))() 
	elseif bool == false then 
	   for i,v in pairs(ws:GetChildren()) do 
		  if v:FindFirstChildOfClass("BodyVelocity") then 
		    if v:FindFirstChildOfClass("BodyGyro") then 
				v:Destroy()
			end
		  end
	   end
   ...
   local tpFolder = maintab:AddFolder("Teleports")
   local tpd = tpFolder:AddDropdown("Teleport to", function(text)
      tptosite(text)
    end)
	for i,v in pairs(game:GetService("Workspace").Spawns.SpawnLocations:GetChildren()) do 
	   tpd:Add(v.Name)
	end
   tpFolder:AddTextBox("TP to player", function(text)
	
		target = text:lower()
		for _, player in ipairs(game:GetService("Players"):GetPlayers()) do 
			if player.Name:lower():sub(1, #target) == target then
				tp(player.Character.HumanoidRootPart.CFrame or game:GetService("Players").LocalPlayer.Character.HumanoidRootPart.CFrame)
				break
			end
		end
 ...
 ["clear"] = false, 
   ...
   local ColorsFolder = s:AddFolder("Theme settings")
   local cp = ColorsFolder:AddColorPicker(function(color)
	if not _G.rainbow then 
	ui_options.main_color = color
	end
    end)
	ColorsFolder:AddSwitch("Rainbow UI",function(var)
		_G.rainbow = var
	end)
	local cred = s:AddFolder("Credits")
	cred:AddLabel("Gui made by MyMomThinksImGay#2392")
    maintab:Show()
  ...
  --[[local function RainbowLoop()
	while wait(.1) do
		...
		if _G.rainbow then 
			for i = 0,1,0.01 do
				ui_options.main_color = Color3.fromHSV(i,1,1)
				wait(.1)
			end
		end
	end
...
local function ToolLoop()
	while wait(.1) do
		if ws.RayIgnore:FindFirstChild(plr.Name) then
		local headgui = ws.RayIgnore[plr.Name].Head.PlayerDisplay
		local hrpgui =  ws.RayIgnore[plr.Name].HumanoidRootPart
		if headgui:FindFirstChild("PlayerName") and headgui:FindFirstChild("TeamName") then 
		   ws.RayIgnore[plr.Name].Head.PlayerDisplay.TeamName:Destroy()
    ...
    if _G.ToolSnipe == true then 
            local lastcf = game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame
            for i,v in pairs(game.Workspace.Entities:GetChildren()) do 
              if v.Name == "ToolModel" then
               if v:FindFirstChild("Handle") then
                  if v.Handle:FindFirstChild("ToolBG") then
                   if checkwl(v) then 
                    --bypass any type of detection methods
                    local randomname = randomString(100)
                    v.Name = randomname
                    --grab tool 
                    repeat
                    game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = game.Workspace.Entities[randomname].Handle.CFrame
                    game:GetService("ReplicatedStorage")["_CS.Events"].Dropper:FireServer(game.Workspace.Entities[randomname], "PickUp")
                    wait(.1)
                    until not game.Workspace.Entities:FindFirstChild(randomname)
                   end
                  end
               end
              end
 ...
 if hrpgui:FindFirstChild("LocalPlayerBG") and hrpgui:FindFirstChild("NameTag") then 
  ...
  notif("GUI", "Loaded",true)
  
  game.Players.LocalPlayer.PlayerGui.Chat.Frame.ChatChannelParentFrame.Visible = true
  game.Players.LocalPlayer.PlayerGui.Chat.Frame.ChatBarParentFrame.Position = UDim2.new(0, 0, 0, 120)
  while wait(.1) do
	NameRemover()
	if _G.rainbow then 
		for i = 0,1,0.01 do
